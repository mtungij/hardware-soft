...
| {{ $item->product?->displayNameWithSize() }} | {{ $item->product?->sku }} | {{ \App\Support\NumberFormatter::quantity($item->ordered_quantity) }} | {{ $item->product?->unit?->short_name }} | {{ \App\Support\NumberFormatter::money($item->cost_price) }} | {{ \App\Support\NumberFormatter::money($item->line_total) }} |
@endforeach
@endcomponent
