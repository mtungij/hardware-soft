...
<div>
    <x-page-header title="Dispensing Stock" description="Current sales counter stock calculated from transfer movements." :breadcrumbs="['Dashboard' => route('dashboard'), 'Dispensing Stock' => null]" />
        ...
        @php
            ...
            $rows = Product::with(['category', 'unit'])
                ...
                ->map(function ($product) use ($inventory, $location, $branchId) {
                    ...
                    $lastTransfer = $location ? StockMovement::where('product_id', $product->id)
                        ->where('stock_location_id', $location->id)
                        ->where('movement_type', 'transfer_in')
                        ->latest('movement_date')
                        ->value('movement_date') : null;
                    return compact('product', 'quantity', 'status', 'lastTransfer');
                })
                ->when($statusFilter, fn ($collection) => $collection->where('status', $statusFilter));
        @endphp

        <x-table :headers="['Product', 'SKU', 'Category', 'Unit', 'Dispensing Qty', 'Reorder', 'Last Transfer', 'Status']">
            @forelse ($rows as $row)
                ...
                <tr class="hover:bg-slate-50 dark:hover:bg-white/5">
                    ...
                    <td class="px-4 py-3">{{ $row['lastTransfer'] ? \Illuminate\Support\Carbon::parse($row['lastTransfer'])->format('d M Y') : '-' }}</td>
            ...
            @empty
                <tr><td colspan="8" class="px-4 py-8 text-center text-slate-500">No dispensing stock found.</td></tr>
            @endforelse
        </x-table>
    </x-card>
</div>
