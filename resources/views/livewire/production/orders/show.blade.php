...
mount(function (ProductionOrder $order): void {
    ...
    $this->order=$order->load(['machine','product','branch','rawMaterialLocation','finishedGoodsLocation','productionOutputLocation','finalFinishedGoodsLocation','curingBatch.qualityInspections','costing','qualityInspections','snapshot.outputUnit','materials.materialProduct','materials.unit','creator','starter','completer','canceller']);
});
...
?>
<div>
    <x-page-header :title="$order->order_number" :description="$order->product?->name.' · '.$order->machine?->name" :breadcrumbs="[__('production.orders.title')=>route('production.orders.index'),$order->order_number=>null]">
