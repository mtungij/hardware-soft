...
<div>
    ...
    <x-card class="mt-4"><x-table :headers="['Branch', 'Location', 'Product', 'Category', 'Qty', 'Avg Cost', 'Value']">@foreach ($rows as $row)<tr><td class="px-4 py-3">{{ $row['branch'] }}</td><td class="px-4 py-3">{{ $row['location'] }}</td><td class="px-4 py-3 font-bold">{{ $row['product'] }}</td><td class="px-4 py-3">{{ $row['category'] }}</td><td class="px-4 py-3 text-right">{{ \App\Support\NumberFormatter::quantity($row['quantity']) }}</td><td class="px-4 py-3 text-right">TZS {{ \App\Support\NumberFormatter::money($row['average_cost']) }}</td><td class="px-4 py-3 text-right font-bold">TZS {{ \App\Support\NumberFormatter::money($row['value']) }}</td></tr>@endforeach</x-table></x-card>
</div>
