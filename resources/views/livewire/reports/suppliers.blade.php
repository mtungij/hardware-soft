...
<div>
    <x-page-header title="Supplier Balance Report" description="Supplier outstanding balances and purchase exposure." :breadcrumbs="['Dashboard' => route('dashboard'), 'Reports' => null, 'Suppliers' => null]"><x-export-actions export="reports.suppliers" :params="compact('search')" /></x-page-header>
    ...
    <div class="mt-4 grid gap-4 sm:grid-cols-3"><x-card><p class="text-sm text-slate-500">Suppliers</p><p class="text-2xl font-black">{{ $rows->count() }}</p></x-card><x-card><p class="text-sm text-slate-500">Outstanding</p><p class="text-2xl font-black text-red-600">TZS {{ \App\Support\NumberFormatter::money($rows->sum('balance')) }}</p></x-card><x-card><p class="text-sm text-slate-500">Opening</p><p class="text-2xl font-black">TZS {{ \App\Support\NumberFormatter::money($rows->sum(fn ($row) => $row['supplier']->opening_balance)) }}</p></x-card></div>
    <x-card class="mt-4"><x-table :headers="['Supplier', 'Phone', 'Opening', 'Outstanding']">@foreach ($rows as $row)<tr><td class="px-4 py-3 font-bold">{{ $row['supplier']->name }}</td><td class="px-4 py-3">{{ $row['supplier']->phone }}</td><td class="px-4 py-3 text-right">TZS {{ \App\Support\NumberFormatter::money($row['supplier']->opening_balance) }}</td><td class="px-4 py-3 text-right font-bold">TZS {{ \App\Support\NumberFormatter::money($row['balance']) }}</td></tr>@endforeach</x-table></x-card>
</div>
