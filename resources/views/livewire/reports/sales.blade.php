...
<div>
    ...
    <div class="mt-4 grid gap-4 sm:grid-cols-4"><x-card><p class="text-sm text-slate-500">Retail Sales Total</p><p class="text-2xl font-black">TZS {{ number_format((float) $retailTotal, 2) }}</p></x-card><x-card><p class="text-sm text-slate-500">Wholesale Sales Total</p><p class="text-2xl font-black">TZS {{ number_format((float) $wholesaleTotal, 2) }}</p></x-card><x-card><p class="text-sm text-slate-500">Retail Profit</p><p class="text-2xl font-black">TZS {{ number_format((float) $retailProfit, 2) }}</p></x-card><x-card><p class="text-sm text-slate-500">Wholesale Profit</p><p class="text-2xl font-black">TZS {{ number_format((float) $wholesaleProfit, 2) }}</p></x-card></div>
    <x-card class="mt-4"><x-table :headers="['Sale Number', 'User', 'Stock Location', 'Sale Type', 'Amount', 'Date', 'Customer', 'Branch']">@foreach ($rows as $sale)<tr><td class="px-4 py-3 font-bold">{{ $sale->sale_number }}</td><td class="px-4 py-3">{{ $sale->soldBy?->name ?? $sale->createdBy?->name }}</td><td class="px-4 py-3">{{ $sale->stockLocationLabel() }}</td><td class="px-4 py-3">{{ $sale->saleTypeLabel() }}</td><td class="px-4 py-3 text-right">TZS {{ number_format((float) $sale->total_amount, 2) }}</td><td class="px-4 py-3">{{ $sale->sale_date?->format('M d, Y') }}</td><td class="px-4 py-3">{{ $sale->customer?->name ?? 'Walk-in' }}</td><td class="px-4 py-3">{{ $sale->branch?->name }}</td></tr>@endforeach</x-table><div class="mt-4">{{ $rows->links() }}</div></x-card>
</div>
