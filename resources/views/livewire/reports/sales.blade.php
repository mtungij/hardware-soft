...
<div>
    ...
    @php
        $canViewCost = auth()->user()->can('sales.view_cost');
        $canViewProfit = auth()->user()->can('sales.view_profit');
        $query = AuthorizationScope::reports(Sale::query(), auth()->user())->with(['branch', 'customer', 'soldBy', 'createdBy', 'items.stockLocation', 'items.product', 'items.sellingUnit', 'items.baseUnit'])->whereDate('sale_date', '>=', $date_from)->whereDate('sale_date', '<=', $date_to)->when($branch_id, fn ($q) => $q->where('branch_id', $branch_id))->when($sale_type, fn ($q) => $q->where('sale_type', $sale_type))->when($stock_location_id, fn ($q) => $q->whereHas('items', fn ($itemQuery) => $itemQuery->where('stock_location_id', $stock_location_id)))->when($search, fn ($q) => $q->where('sale_number', 'like', "%{$search}%")->orWhereHas('customer', fn ($c) => $c->where('name', 'like', "%{$search}%")));
        ...
        $saleItemQuery = \App\Models\SaleItem::query()
            ->whereHas('sale', fn ($saleQuery) => AuthorizationScope::reports($saleQuery, auth()->user())->where('status', 'completed')->whereDate('sale_date', '>=', $date_from)->whereDate('sale_date', '<=', $date_to)->when($branch_id, fn ($q) => $q->where('branch_id', $branch_id)))
    ...
    <div class="mt-4 grid gap-4 sm:grid-cols-4"><x-card><p class="text-sm text-slate-500">Retail Sales Total</p><p class="text-2xl font-black">TZS {{ \App\Support\NumberFormatter::money($retailTotal) }}</p></x-card><x-card><p class="text-sm text-slate-500">Wholesale Sales Total</p><p class="text-2xl font-black">TZS {{ \App\Support\NumberFormatter::money($wholesaleTotal) }}</p></x-card>@if ($canViewProfit)<x-card><p class="text-sm text-slate-500">Retail Profit</p><p class="text-2xl font-black">TZS {{ \App\Support\NumberFormatter::money($retailProfit) }}</p></x-card><x-card><p class="text-sm text-slate-500">Wholesale Profit</p><p class="text-2xl font-black">TZS {{ \App\Support\NumberFormatter::money($wholesaleProfit) }}</p></x-card>@endif</div>
    <x-card class="mt-4">
        <x-table :headers="array_values(array_filter(['Sale Number', 'Products / Transaction Qty', 'Base Qty Sold', 'User', 'Stock Location', 'Sale Type', 'Sales Total', $canViewCost ? 'COGS' : null, $canViewProfit ? 'Gross Profit' : null, 'Date', 'Customer', 'Branch']))">
            @foreach ($rows as $sale)
                @php($saleCogs = ($canViewCost || $canViewProfit) ? $sale->items->sum(fn ($item) => $item->base_unit_cost !== null ? (float) $item->base_quantity * (float) $item->base_unit_cost : (float) $item->quantity * (float) $item->unit_cost) : 0)
                <tr>
                    ...
                    <td class="px-4 py-3 text-right">TZS {{ \App\Support\NumberFormatter::money($sale->total_amount) }}</td>@if ($canViewCost)<td class="px-4 py-3 text-right">TZS {{ \App\Support\NumberFormatter::money($saleCogs) }}</td>@endif @if ($canViewProfit)<td class="px-4 py-3 text-right">TZS {{ \App\Support\NumberFormatter::money((float) $sale->total_amount - $saleCogs) }}</td>@endif
