...
<div>
    ...
    @php
        ...
        $saleItemQuery = \App\Models\SaleItem::query()
            ->whereHas('sale', fn ($saleQuery) => AuthorizationScope::reports($saleQuery, auth()->user())->where('status', 'completed')->whereBetween('sale_date', [$date_from, $date_to])->when($branch_id, fn ($q) => $q->where('branch_id', $branch_id)))
