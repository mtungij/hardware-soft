...
<div>
    ...
    <x-card class="mt-4"><x-table :headers="['Sale Number', 'User', 'Stock Location', 'Sale Type', 'Amount', 'Date', 'Customer', 'Branch']">@foreach ($rows as $sale)<tr><td class="px-4 py-3 font-bold">{{ $sale->sale_number }}</td><td class="px-4 py-3">{{ $sale->soldBy?->name ?? $sale->createdBy?->name }}</td><td class="px-4 py-3">{{ $sale->stockLocationLabel() }}</td><td class="px-4 py-3">{{ $sale->saleTypeLabel() }}</td><td class="px-4 py-3 text-right">TZS {{ \App\Support\NumberFormatter::money($sale->total_amount) }}</td><td class="px-4 py-3">{{ $sale->sale_date?->format('M d, Y') }}</td><td class="px-4 py-3">{{ $sale->customer?->name ?? 'Walk-in' }}</td><td class="px-4 py-3">{{ $sale->branch?->name }}</td></tr>@endforeach</x-table><div class="mt-4">{{ $rows->links() }}</div></x-card>
</div>
