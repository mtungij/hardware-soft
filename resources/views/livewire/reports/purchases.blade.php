...
<div>
    ...
    <x-card class="mt-4"><x-table :headers="['Date', 'Reference', 'Supplier', 'Status', 'Total', 'Paid', 'Balance']">@foreach ($rows as $purchase)<tr><td class="px-4 py-3">{{ $purchase->purchase_date?->format('M d, Y') }}</td><td class="px-4 py-3 font-bold">{{ $purchase->reference_number }}</td><td class="px-4 py-3">{{ $purchase->supplier?->name }}</td><td class="px-4 py-3">{{ ucfirst($purchase->payment_status) }}</td><td class="px-4 py-3 text-right">TZS {{ \App\Support\NumberFormatter::money($purchase->total_amount) }}</td><td class="px-4 py-3 text-right">TZS {{ \App\Support\NumberFormatter::money($purchase->paid_amount) }}</td><td class="px-4 py-3 text-right">TZS {{ \App\Support\NumberFormatter::money($purchase->balance_amount) }}</td></tr>@endforeach</x-table></x-card>
</div>
